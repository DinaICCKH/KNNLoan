...
var options = "";
//To-Bin
function cmd_inv_trn_to_bin_click(summaryid) {
    var whscode = $("#txt_line_pr_to_whs_code_" + summaryid).val();
    var whsname = $("#txt_line_pr_to_whs_code_" + summaryid).find(":selected").text();
    $("#modal-to_bin_list").modal('show');
    var found = 0;
    pop_to_bing_check();
    $("#table_pop_to_whs_bin >tbody >tr").each(function () {
    ...
    if (found == 0) {
        $("#table_bin > tbody > tr").each(function () {
            if ($(this).find("td:eq(3)").text() == whscode) {
                ...
                tr = tr + "</tr>";
                $("#table_pop_to_whs_bin >tbody").append(tr);
            }
        });
    }
    $("#table_pop_to_whs_bin >tbody >tr").each(function () {
...
function pop_to_bin_quantity_change(summaryid) {
...
function cmd_pop_update_to_bin() {
    $("#table_pop_whs_bin >tbody >tr").each(function (index, tr) {
...
//From BIN
function cmd_inv_trn_from_bin_click(summaryid) {
...
function pop_from_bin_quantity_change(summaryid) {
    $("#table_pop_whs_bin >tbody >tr").each(function (index, tr) {
        if ($(this).find("td:eq(0)").text() == summaryid) {
            var qty = $(this).find("td:eq(6) input[type='text']").val();
            if (qty != "") {
...
function cmd_pop_update_from_bin() {
    $("#table_pop_whs_bin >tbody >tr").each(function (index, tr) {
        $(this).find("td:eq(1)").text("Updated");
    });
    $("#modal-bin_list").modal('hide');
        ...
        if (this.checked) {
            ...
            if ($("#txtselectrowindex").val() != "999") {
                ...
                $("#table_whs >tbody>tr").each(function () {
                    if ($(this).find("td:eq(0)").text() == $("#td_pq_pop_base_from_whs_code_" + id).text()) {
                        fromwhs = fromwhs + "<option value='" + $(this).find("td:eq(0)").text() + "' selected=selected>" + $(this).find("td:eq(1)").text() + "</option>";
                        fromenablebin = $(this).find("td:eq(3)").text();
                    }
                    else {
                        fromwhs = fromwhs + "<option value='" + $(this).find("td:eq(0)").text() + "'>" + $(this).find("td:eq(1)").text() + "</option>";
                    }
                });
                $("#table_whs >tbody>tr").each(function () {
                    if ($(this).find("td:eq(0)").text() == $("#td_pq_pop_base_to_whs_code_" + id).text()) {
                        towhs = towhs + "<option value='" + $(this).find("td:eq(0)").text() + "' selected=selected>" + $(this).find("td:eq(1)").text() + "</option>";
                        toenablebin = $(this).find("td:eq(3)").text();
                    }
                    else {
                        towhs = towhs + "<option value='" + $(this).find("td:eq(0)").text() + "'>" + $(this).find("td:eq(1)").text() + "</option>";
                    }
                });
                ...
                tr = tr + "<td><div class='form-group'><select class='form-control' name='whs' id='txt_line_pr_from_whs_code_" + rowindex + "' onchange='change_line_from_whs_pq(" + rowindex + ")'>" + fromwhs + "</select></div></td>";
                if (fromenablebin == "Y") {
                    tr = tr + "<td><div class='form-group'><button type='button' class='btn btn-default btn-flat' id='btn_line_pr_from_bin_" + rowindex + "' onclick='cmd_inv_trn_from_bin_click(" + rowindex + ")'>...</button></div></td>";
                }
                else {
                    tr = tr + "<td><div class='form-group'><button type='button' style='display:none' class='btn btn-default btn-flat' id='btn_line_pr_from_bin_" + rowindex + "' onclick='cmd_inv_trn_from_bin_click(" + rowindex + ")'>...</button></div></td>";
                }
                tr = tr + "<td><div class='form-group'><select class='form-control' name='whs' id='txt_line_pr_to_whs_code_" + rowindex + "' onchange='change_line_to_whs_pq(" + rowindex + ")'>" + towhs + "</select></div></td>";
                if (toenablebin == "Y") {
                    tr = tr + "<td><div class='form-group'><button type='button' class='btn btn-default btn-flat' id='btn_line_pr_to_bin_" + rowindex + "' onclick='cmd_inv_trn_to_bin_click(" + rowindex + ")'>...</button></div></td>";
                }
                else {
                    tr = tr + "<td><div class='form-group'><button type='button' style='display:none' class='btn btn-default btn-flat' id='btn_line_pr_to_bin_" + rowindex + "' onclick='cmd_inv_trn_to_bin_click(" + rowindex + ")'>...</button></div></td>";
                }
...
function change_line_from_whs_pq(id) {
    var isbin = "";
    $("#table_whs > tbody>tr").each(function () {
        if ($("#txt_line_pr_from_whs_code_" + id).val() == $(this).find("td:eq(0)").text()) {
            isbin = $(this).find("td:eq(3)").text();
            return false;
        }
    });
    if (isbin == "N") {
        $("#btn_line_pr_from_bin_" + id).hide();
    } else {
        $("#btn_line_pr_from_bin_" + id).show();
    }
}

function change_line_to_whs_pq(id) {
    var isbin = "";
    $("#table_whs > tbody>tr").each(function () {
        if ($("#txt_line_pr_to_whs_code_" + id).val() == $(this).find("td:eq(0)").text()) {
            isbin = $(this).find("td:eq(3)").text();
            return false;
        }
    });
    if (isbin == "N") {
        $("#btn_line_pr_to_bin_" + id).hide();
    } else {
        $("#btn_line_pr_to_bin_" + id).show();
    }
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < data.data.length; i++) {
                ...
                tr = tr + "<td style='display:none' id='td_pq_pop_base_from_whs_code_" + i + "'>" + x.FromWhsCode + "</td>";
                tr = tr + "<td style='display:none' id='td_pq_pop_base_to_whs_code_" + i + "'>" + x.ToWhsCode + "</td>";
                ...
                tr = tr + "<td id='td_pq_pop_base_ocrcode3name_" + i + "'>" + x.Quantity + "</td>";
                tr = tr + "<td id='td_pq_pop_base_f_whs_name_" + i + "'>" + x.FWhsName + "</td>";
                tr = tr + "<td id='td_pq_pop_base_t_whs_name_" + i + "'>" + x.TWhsName + "</td>";
                tr = tr + "</tr>";
